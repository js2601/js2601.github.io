...
function getMousePos(event) { //Function chatGPT helped me make with the prompt: "How would I find the cursor position using canvasAPI?"
    const rect = canvas.getBoundingClientRect();
    const x = event.clientX;
    const y = event.clientY;
    mousePos = [x-rect.left,y-rect.top];
...
function drawBoard() {
    ...
    for (let i = 0; i <3; i++) {
        ctx.strokeRect(boardLeft+boardWidth/3*i,boardTop+boardHeight,boardWidth/3,boardHeight/3);
        if (i == 0) {
            ctx.fillText(`1st 12`,boardLeft+boardWidth/3*i+boardWidth/6,boardTop+7*boardHeight/6);
        }
        else if (i == 1) {
            ctx.fillText(`2nd 12`,boardLeft+boardWidth/3*i+boardWidth/6,boardTop+7*boardHeight/6);
        }
        else {
            ctx.fillText(`3rd 12`,boardLeft+boardWidth/3*i+boardWidth/6,boardTop+7*boardHeight/6);
        }
    }
    for (let i = 0; i < 6; i++) {
